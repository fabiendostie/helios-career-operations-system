...
from ..models.text_analysis import TextAnalysis, ReadabilityMetrics, ContentAnalysis
...
class TextOptimizer:
    ...
    def _load_models(self):
        ...
        try:
            ...
            logger.warning(f"Could not load {settings.SPACY_MODEL_EN}, using basic model")
            self.nlp_en = None
    ...
    def optimize_text(self, request: EditRequest) -> EditResponse:
        ...
        if request.edit_type in [EditType.STYLE, EditType.COMPREHENSIVE, EditType.TONE_ADJUSTMENT]:
        ...
        if request.edit_type == EditType.QUANTIFICATION:
            optimized_text, quant_suggestions = self._apply_quantification(optimized_text)
    ...
    def _analyze_content(self, text: str, language: str) -> ContentAnalysis:
        ...
        # Use spaCy for analysis if available
        ...
        quantified_pattern = r'\b\d+(?:[\.,]\d+)?(?:%|percent|k|thousand|million|billion)?\b'
        quantified_achievements = len(re.findall(quantified_pattern, text, re.IGNORECASE))
    ...
    def _count_action_verbs(self, text: str) -> int:
        ...
        action_verbs = {
            ...
            'optimized', 'organized', 'reduced', 'resolved', 'streamlined'
        }
    ...
    def _count_weak_words(self, text: str) -> int:
        ...
        weak_phrases = {
            ...
            'involved in', 'participated in', 'contributed to', 'did',
            'made', 'got', 'things', 'stuff', 'various', 'several'
        }
    ...
    def _extract_industry_keywords(self, text: str) -> List[str]:
        ...
        tech_keywords = {
            'api', 'database', 'algorithm', 'framework', 'software',
            'development', 'programming', 'system', 'application',
    ...
    def _apply_grammar_fixes(self, text: str, grammar_issues) -> tuple[str, List[EditSuggestion]]:
    ...
    def _apply_style_improvements(self, text: str, style_issues, tone: Optional[str]) -> tuple[str, List[EditSuggestion]]:
    ...
    def _apply_content_enhancements(self, text: str, industry: Optional[str], role: Optional[str]) -> tuple[str, List[EditSuggestion]]:
            ...
            if weak in enhanced_text.lower():
                ...
                if match:
                    suggestion = EditSuggestion(
                        ...
                        reason=f"Replace weak phrase with stronger action verb",
                        confidence=0.9,
