...
from src.core.document_generator import (
    DocumentGenerator, MemoryEfficientPDFGenerator, DOCXGenerator,
    DocumentGenerationError, monitor_memory_usage
)
...
class TestMemoryEfficientPDFGenerator:
    ...
    async def test_pdf_generation_success(self, pdf_generator):
        ...
        html_content = "<html><body><h1>Test Document</h1><p>Content here</p></body></html>"
    ...
    async def test_pdf_generation_memory_error_recovery(self, pdf_generator):
        ...
        with patch('weasyprint.HTML') as mock_html:
            ...
            mock_html.side_effect = [MemoryError("Out of memory"), Mock()]
            
            with patch.object(pdf_generator, '_generate_minimal_pdf') as mock_minimal:
...
class TestDOCXGenerator:
    ...
    def sample_career_data(self):
        ...
        return {
            ...
            'strategic_metadata': {
                'core_competencies': ['Machine Learning', 'Data Analysis', 'Team Leadership'],
    ...
    async def test_docx_generation_success(self, docx_generator, sample_career_data):
        ...
        with patch('docx.Document') as mock_doc_class:
            ...
            with patch('tempfile.NamedTemporaryFile') as mock_temp:
                ...
                result = await docx_generator.generate_docx_from_data(sample_career_data)
    ...
    @pytest.mark.asyncio
    async def test_docx_generation_error_handling(self, docx_generator, sample_career_data):
...
class TestDocumentGenerator:
    ...
    def sample_career_data(self):
        ...
        return {
            ...
            'skills_inventory': {
                'technical_skills': {
                    'marketing': ['Google Ads', 'Analytics', 'CRM']
                }
            }
        }
    ...
    async def test_html_document_generation(self, doc_generator, sample_career_data):
        ...
        with patch.object(doc_generator.template_engine, 'render_resume_template') as mock_render:
    ...
    async def test_pdf_document_generation(self, doc_generator, sample_career_data):
        ...
        with patch.object(doc_generator.template_engine, 'render_resume_template') as mock_render:
            ...
            with patch.object(doc_generator.pdf_generator, 'memory_managed_pdf_conversion') as mock_pdf:
    ...
    @pytest.mark.asyncio
    async def test_markdown_document_generation(self, doc_generator, sample_career_data):
    ...
    async def test_docx_document_generation(self, doc_generator, sample_career_data):
        ...
        with patch.object(doc_generator.docx_generator, 'generate_docx_from_data') as mock_docx:
    ...
    async def test_cover_letter_generation(self, doc_generator, sample_career_data):
        ...
        with patch.object(doc_generator.template_engine, 'render_cover_letter_template') as mock_render:
    ...
    @pytest.mark.asyncio
    async def test_unsupported_document_type_error(self, doc_generator, sample_career_data):
    ...
    async def test_output_format_validation(self, doc_generator):
        ...
        with patch.object(doc_generator.settings, 'supported_formats', ['html', 'pdf', 'docx']):
            assert await doc_generator.validate_output_format('html') is True
            assert await doc_generator.validate_output_format('PDF') is True  # Case insensitive
        ...
        with patch('yaml.safe_load', return_value=mock_config):
            with patch('os.path.exists', return_value=True):
                with patch('builtins.open', mock_open(read_data="mock yaml")):
                    template_info = await doc_generator.get_template_info('t_shaped_classic')
        ...
        with patch('yaml.safe_load', return_value=mock_config):
            with patch('os.path.exists', return_value=True):
                with patch('builtins.open', mock_open(read_data="mock yaml")):
                    ...
                    resume_templates = await doc_generator.list_available_templates('resume')
                    assert len(resume_templates) == 1
                    assert resume_templates[0]['id'] == 't_shaped_classic'
                    
                    cover_letter_templates = await doc_generator.list_available_templates('cover_letter')
