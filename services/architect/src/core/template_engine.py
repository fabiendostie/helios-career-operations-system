...
from .config import PERFORMANCE_THRESHOLDS, TEMPLATES_DIR, get_settings
...
class MemoryOptimizedTemplateEngine:
    ...
    def _format_duration(self, start_date: str, end_date: str | None = None) -> str:
    ...
    def get_compiled_template(self, template_path: str) -> Template:
        ...
        try:
            ...
            logger.error(
                "Failed to compile template", template_path=template_path, error=str(e)
            )
    ...
    async def render_template_with_memory_management(
        self, template_path: str, context: dict[str, Any], enable_streaming: bool = True
    ) -> str:
        """
        ...
        try:
            ...
            if context_size > 50:  # >50MB estimated
                logger.info(
                    "Using streaming render for large context",
            ...
            logger.warning(
                "Memory error during template rendering, attempting recovery"
            )
    ...
    async def warm_cache(self, template_paths: list[str] | None = None):
        ...
        for template_path in template_paths:
            try:
                ...
                logger.warning(
                    "Failed to warm template", template_path=template_path, error=str(e)
                )
...
class TemplateEngine:
    ...
    async def render_resume_template(
        ...
        try:
            rendered_content = (
                await self._memory_engine.render_template_with_memory_management(
                    template_path, context
                )
            )
    ...
    def _prepare_resume_context(
        ...
        context["projects"] = career_data.get("projects", [])
        context["certifications"] = career_data.get("certifications", [])
        context["career_data"] = career_data  # For template backward compatibility
        ...
        context["quantified_achievements"] = strategic_metadata.get(
    ...
    def _prepare_cover_letter_context(
        ...
        context.update(
            self._prepare_pain_promise_context(
                career_data, job_requirements, company_data
            )
        )
    ...
    def _prepare_t_shaped_context(
        ...
        top_achievement = (
            achievements[0] if achievements else "delivered exceptional results"
        )
        ...
        primary_skill_area = (
            list(technical_skills.keys())[0] if technical_skills else "technology"
        )
        ...
        return {
            ...
            "core_competency_1": (
                core_competencies[0] if len(core_competencies) > 0 else "expertise"
            ),
            "core_competency_2": (
                core_competencies[1] if len(core_competencies) > 1 else "leadership"
            ),
            "core_technology": (
                core_competencies[2] if len(core_competencies) > 2 else "innovation"
            ),
    ...
    def _prepare_pain_promise_context(
        ...
        achievements = strategic_metadata.get("quantified_achievements", [])
        top_achievement = (
            achievements[0] if achievements else "delivered significant results"
        )

        # Extract relevant experience for bridge
        work_experience = career_data.get("work_experience", [])
        previous_company = (
            work_experience[0]["company_name"] if work_experience else "previous role"
        )
