...
async def generate_integrated_document(
    ...
    logger.info("Starting integrated document generation",
    ...
    try:
        ...
        session_data = await service_clients.orchestrator.get_session(request.session_id)
        if not session_data:
            raise HTTPException(
                status_code=404,
        ...
        if not user_profile:
            raise HTTPException(
                status_code=400,
                detail="No user profile data available in session"
            )
        ...
        template_context = {
            ...
            "analyst_recommendations": analyst_recommendations.model_dump() if analyst_recommendations else {},
            "strategist_insights": strategist_insights.model_dump() if strategist_insights else {},
            ...
            "generation_metadata": {
                ...
                "services_used": {
                    ...
                    "research_engine": bool(industry_intelligence or company_intelligence)
                }
            }
        }
        ...
        if request.include_validation and temp_file:
            ...
            validation_results = {
                ...
                "vendor_scores": {v.value: s for v, s in validation_result.ats_vendor_scores.items()}
            }
            compliance_score = validation_result.compliance_score
        ...
        logger.info("Integrated document generation completed",
                   request_id=request_id,
                   session_id=request.session_id,
        ...
        response = DocumentGenerationResponse(
            ...
            compliance_violations=validation_results.get("violations", []) if validation_results else [],
            recommendations=validation_results.get("recommendations", []) if validation_results else [],
            template_used=f"{request.document_type.value}_{request.architecture.value}",
            research_sources=_extract_research_sources(industry_intelligence, company_intelligence),
            generation_metadata={
                "services_coordinated": ["orchestrator", "analyst", "strategist"],
                "intelligence_sources": len(_extract_research_sources(industry_intelligence, company_intelligence)),
        ...
        logger.error("Integrated document generation failed",
                    request_id=request_id,
                    session_id=request.session_id,
                    error=str(e),
...
async def _cleanup_temp_file(file_path: Path):
    ...
    try:
        ...
        logger.warning("Failed to cleanup temp file", file_path=str(file_path), error=str(e))

def _calculate_content_quality_score(
    context: Dict[str, Any],
    analyst_recs: Any,
    strategist_insights: Any
) -> float:
