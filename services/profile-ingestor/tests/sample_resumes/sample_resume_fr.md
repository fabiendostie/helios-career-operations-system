# Marie Dubois
## Ingénieure Logiciel Senior

**Email:** marie.dubois@email.com
**Téléphone:** +33 1 23 45 67 89

## Expérience Professionnelle

### Développeuse Principal | Société Générale | 2019-2023
- Direction d'équipe de 8 développeurs sur des projets bancaires critiques
- Implémentation d'architecture microservices avec Spring Boot et Docker
- Amélioration des performances système de 40%
- Technologies: Java, Spring Boot, Docker, PostgreSQL, React

### Développeuse Full-Stack | BNP Paribas | 2017-2019
- Développement d'applications web pour la gestion de portefeuille
- Collaboration étroite avec les équipes métier
- Technologies: JavaScript, Node.js, MongoDB, Vue.js

## Formation

**Master en Informatique**
École Centrale Paris | 2015-2017

**Licence en Mathématiques Appliquées**
Université Pierre et Marie Curie | 2012-2015

## Compétences

**Langages de programmation:** Java, JavaScript, Python, TypeScript
**Frameworks:** Spring Boot, Vue.js, React, Node.js
**Bases de données:** PostgreSQL, MongoDB, Oracle
**Outils:** Docker, Jenkins, Git, Maven, Kubernetes
