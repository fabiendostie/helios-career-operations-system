...
class ConsolidationEngine:
    ...
    def _initialize_base_structure(self, base_doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Handle both dict and ParsedData objects
        if hasattr(base_doc, '__dict__'):
            # It's a ParsedData object, convert to dict
            return {
                "personal_info": self._extract_personal_info(base_doc),
                "work_experience": getattr(base_doc, 'work_experiences', []),
                "education": getattr(base_doc, 'education', []),
                "skills": getattr(base_doc, 'skills', []),
                "projects": getattr(base_doc, 'projects', []),
                "source_documents": [getattr(base_doc, 'source_file', 'unknown')],
            }
        else:
            # It's already a dict
            return {
    ...
    def _extract_personal_info(self, document_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hasattr(document_data, '__dict__'):
            # It's a ParsedData object
            entities = getattr(document_data, 'raw_entities', {})
            contact = getattr(document_data, 'contact_info', {})
        else:
            # It's a dict
    ...
    def _merge_document(
        ...
        # Handle both dict and ParsedData objects
        if hasattr(new_doc, '__dict__'):
            # It's a ParsedData object
            source_file = getattr(new_doc, 'source_file', f"document_{doc_index}")
            work_exp = getattr(new_doc, 'work_experiences', [])
            education = getattr(new_doc, 'education', [])
            skills = getattr(new_doc, 'skills', [])
            projects = getattr(new_doc, 'projects', [])
        else:
            # It's a dict
            source_file = new_doc.get("source_file", f"document_{doc_index}")
            work_exp = new_doc.get("work_experience", [])
            education = new_doc.get("education", [])
            skills = new_doc.get("skills", [])
            projects = new_doc.get("projects", [])

        # Add to source documents
        consolidated["source_documents"].append(source_file)

        # Merge work experience
        consolidated["work_experience"].extend(work_exp)

        # Merge education
        consolidated["education"].extend(education)

        # Merge skills
        consolidated["skills"].extend(skills)

        # Merge projects
        consolidated["projects"].extend(projects)
